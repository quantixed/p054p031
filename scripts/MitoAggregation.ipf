...
// Need to have Area and Perim. as columns
// looking for TPD54 and R159E (will need editing for other conditions)

////////////////////////////////////////////////////////////////////////
// Menu items
////////////////////////////////////////////////////////////////////////
Menu "Macros"
	"Mitochondrial Aggregation...",  ParticleAnalysis()
...
Function LoadAndProcessCSVs(fWaveToLoad, pathNameString, condWave)
	...
	for(i = 0; i <= nCond; i += 1)
		...
		Make/O/N=(nWaves) $("root:summary" + num2str(i))
		Wave w0 = $("root:summary" + num2str(i))
	
		for (j = 0; j < nWaves; j += 1)
			wName = StringFromList(j, wList)
			Wave w = $wName
			MatrixOP/O/FREE tempW = col(w,2)
			w0[j] = median(tempW)
		endfor
	endfor
	...
	KillWindow/Z p_summary
	Display/N=p_summary
	Wave w0 = $("root:summary" + num2str(0))
	AppendBoxPlot/W=p_summary w0 vs condWave
	for(i = 1; i < nCond; i += 1)
		// any mismatches won't get plotted
		Wave w1 = $("root:summary" + num2str(i))
		if(WaveExists(w1))
			AddWavesToBoxPlot/W=p_summary w1
		endif
	endfor
	SetAxis/W=p_summary left 0,1
	Label/W=p_summary left "Circularity"
End
